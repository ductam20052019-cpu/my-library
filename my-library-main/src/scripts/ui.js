...
function getSectionAccessUser() {
  ...
  if (isStaffRole(liveUser.role)) {
    ...
    const hasFirebaseSession =
      !!authUser &&
      (
        (liveUser.authUid && liveUser.authUid === authUser.uid) ||
        (liveUser.email && String(liveUser.email).toLowerCase() === String(authUser.email || "").toLowerCase())
      );
    if (!hasFirebaseSession || liveUser.mustChangePassword) return null;
  }
...
function buildBookCoverHtml(book) {
  const title = String(book?.title || "Sách").trim() || "Sách";
