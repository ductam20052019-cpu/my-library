...
const deleteFieldFn = window.fs.deleteField;
const queryFn = window.fs.query;
const whereFn = window.fs.where;
const db = window.db;
const authApi = window.authApi || {};
const auth = authApi.auth || null;
const accountCreationAuth = authApi.accountCreationAuth || auth;
...
const onAuthStateChangedFn = authApi.onAuthStateChanged;
...
let legacyPasswordVerifiedForMigration = false;
let legacyPasswordMigrationUserId = "";
let legacyAccountBackfillStarted = false;
...
function isAdminBootstrapUsername(username) {
  return normalizeText(username).toUpperCase() === ADMIN_BOOTSTRAP.indexId;
}

function isAdminBootstrapAuthUser(authUser) {
  return authUser?.uid === ADMIN_BOOTSTRAP.authUid
    && normalizeEmail(authUser?.email) === ADMIN_BOOTSTRAP.email;
}

function sanitizeUserProfile(user = {}, fallbackUsername = "") {
  const clean = { ...(user || {}) };
  delete clean.pass;
  delete clean.password;
  delete clean.passwordHash;
  if (fallbackUsername && !clean.username) clean.username = fallbackUsername;
  if (clean.email) clean.email = normalizeEmail(clean.email);
  return clean;
}

function setCurrentUserSession(user = {}, fallbackUsername = "", options = {}) {
  currentUser = sanitizeUserProfile(user, fallbackUsername);
  syncState();
...
function clearStoredPasswordPatch(extra = {}) {
  const patch = {
    ...extra,
  ...
  if (typeof deleteFieldFn === "function") {
    patch.pass = deleteFieldFn();
    patch.password = deleteFieldFn();
    patch.passwordHash = deleteFieldFn();
  }
  return patch;
}

async function removeLegacyPasswordField(userId, extra = {}) {
  if (!userId || typeof deleteFieldFn !== "function") return;
  try {
    await updateDoc(doc(db, "users", userId), clearStoredPasswordPatch(extra));
  } catch (err) {
    console.warn("Could not remove legacy password field:", err);
  }
}

async function findUsernameIndex(rawUsername) {
  const userId = normalizeText(rawUsername).toUpperCase();
  if (!userId) return null;
  try {
    const snap = await getDoc(doc(db, "usernames", userId));
    if (!snap.exists()) return null;
    return { id: userId, data: snap.data() || {} };
  } catch (err) {
    console.warn("Could not read username index:", err);
    if (isPermissionDeniedError(err)) throw err;
    return null;
  }
}

async function ensureUsernameIndex(userId, email, authUid = "", role = "") {
  const profileUserId = normalizeText(userId);
  const normalizedUserId = profileUserId.toUpperCase();
  const normalizedEmail = normalizeEmail(email);
  if (!normalizedUserId || !isValidEmail(normalizedEmail)) return;

  try {
    await setDoc(doc(db, "usernames", normalizedUserId), {
      userId: profileUserId,
      email: normalizedEmail,
      authUid: normalizeText(authUid),
...
async function ensureAuthUserIndex(authUid, userId, email = "", role = "") {
  const normalizedAuthUid = normalizeText(authUid);
  const profileUserId = normalizeText(userId);
  if (!normalizedAuthUid || !profileUserId) return;

  try {
    await setDoc(doc(db, "authUsers", normalizedAuthUid), {
      userId: profileUserId,
    ...
    console.warn("Could not update auth user index:", err);
  }
}
...
function getLegacyPassword(user = {}) {
  const raw = user.pass ?? user.password ?? "";
  return normalizeText(raw);
}

async function resolveLegacyAuthUser(email, password) {
  if (
    !accountCreationAuth ||
    accountCreationAuth === auth ||
    typeof createUserWithEmailAndPasswordFn !== "function" ||
    typeof signInWithEmailAndPasswordFn !== "function"
  ) {
    return null;
  }

  try {
    const credential = await createUserWithEmailAndPasswordFn(accountCreationAuth, email, password);
    return credential?.user || accountCreationAuth.currentUser || null;
  } catch (createErr) {
    const code = String(createErr?.code || "");
    if (code !== "auth/email-already-in-use") throw createErr;

    const credential = await signInWithEmailAndPasswordFn(accountCreationAuth, email, password);
    return credential?.user || accountCreationAuth.currentUser || null;
  } finally {
    if (typeof signOutFn === "function" && accountCreationAuth?.currentUser) {
      try {
        ...
        console.warn("Could not sign out secondary auth during backfill:", signOutErr);
      }
    }
  }
}

async function backfillLegacyAccountsForAdmin() {
  if (legacyAccountBackfillStarted || currentUser?.role !== "admin") return;
  legacyAccountBackfillStarted = true;
  ...
  try {
    ...
    for (const item of snapshot.docs) {
      ...
      const role = normalizeText(data.role || "student");
      const legacyPassword = getLegacyPassword(data);
      ...
      let authUid = normalizeText(data.authUid);
      if (authUid && data.legacyBackfilledAt && !legacyPassword) {
        continue;
      }
      ...
      if (!authUid) {
        ...
        try {
          const authUser = await resolveLegacyAuthUser(email, legacyPassword);
          authUid = normalizeText(authUser?.uid);
        } catch (authErr) {
          console.warn(`Could not backfill auth for ${userId}:`, authErr);
          skipped += 1;
          continue;
        }
      }
      ...
      await removeLegacyPasswordField(userId, {
        ...
        legacyBackfilledAt: Date.now()
      });
      changed += 1;
    }

    if (changed > 0) {
      window.showToast?.(`Đã đồng bộ ${changed} tài khoản cũ sang Firebase Auth.`, "success");
    }
    if (skipped > 0) {
      console.info(`Skipped ${skipped} legacy accounts during auth backfill.`);
    }
  } catch (err) {
    console.warn("Legacy account backfill failed:", err);
    legacyAccountBackfillStarted = false;
  }
}

function scheduleLegacyBackfillForAdmin() {
  if (currentUser?.role !== "admin" || legacyAccountBackfillStarted) return;
  window.setTimeout(() => {
    backfillLegacyAccountsForAdmin();
  }, 800);
}

function schedulePostLoginMaintenance({ userId, email, authUid, role, patch = {} }) {
  if (!userId || !authUid) return;
  const normalizedRole = normalizeText(role);

  Promise.all([
    removeLegacyPasswordField(userId, {
      authUid,
...
async function findUserDocByEmail(rawEmail) {
  ...
  if (typeof queryFn === "function" && typeof whereFn === "function") {
    try {
      const q = queryFn(usersCollection, whereFn("email", "==", targetEmail));
      ...
      console.warn("Could not query user by email, falling back to full scan:", err);
    }
  }
...
window.requestBorrow = async function(bookId, triggerBtn) {
  ...
  const run = async () => {
    await addDoc(loansCollection, {
      ...
      userAuthUid: currentUser.authUid || auth?.currentUser?.uid || "",
      bookId: bookId,
...
window.sendResetPasswordEmail = async function(triggerBtn) {
            const run = async () => {
              ...
              const tryBootstrapAuthAccount = async () => {
                ...
                const canBootstrapAuth = accountCreationAuth && typeof createUserWithEmailAndPasswordFn === "function";
                if (!canBootstrapAuth) return false;

                try {
                  const tempPassword = generateBootstrapPassword();
                  await createUserWithEmailAndPasswordFn(accountCreationAuth, email, tempPassword);
                } catch (bootstrapErr) {
                  ...
                  if (
                    ...
                    accountCreationAuth?.currentUser &&
                    normalizeEmail(accountCreationAuth.currentUser.email || "") === email
                  ) {
                    try {
                      await signOutFn(accountCreationAuth);
                    } catch (signErr) {
              ...
              if (profile) {
                await removeLegacyPasswordField(profile.id, {
                  authBootstrappedAt: Date.now(),
                  resetEmailSentAt: Date.now()
                });
              }
...
window.handleLogin = async function() {
            ...
            if (!auth || typeof signInWithEmailAndPasswordFn !== "function") {
                return alert("Chưa cấu hình Firebase Authentication cho đăng nhập.");
            }
            ...
            try {
                const normalizedUsername = normalizeText(u).toUpperCase();
                let usernameIndex = await findUsernameIndex(normalizedUsername);
                if (!usernameIndex && isAdminBootstrapUsername(normalizedUsername)) {
                    usernameIndex = {
                        id: ADMIN_BOOTSTRAP.indexId,
                        data: {
                            userId: ADMIN_BOOTSTRAP.username,
                            email: ADMIN_BOOTSTRAP.email,
                            authUid: ADMIN_BOOTSTRAP.authUid,
                            role: "admin"
                        }
                    };
                }
                let matchedUserId = usernameIndex?.data?.userId || usernameIndex?.id || normalizedUsername;
                let userData = null;
                let accountEmail = normalizeEmail(usernameIndex?.data?.email);
                let authUser = null;
                let requiresPasswordChange = false;

                if (usernameIndex && isValidEmail(accountEmail)) {
                    try {
                        const credential = await signInWithEmailAndPasswordFn(auth, accountEmail, p);
                        authUser = credential?.user || auth.currentUser || null;
                    } catch (signErr) {
                        showAuthError(signErr);
                        return;
                    }
                    const profileSnap = await getDoc(doc(db, "users", matchedUserId));
                    if (!profileSnap.exists()) {
                        if (isAdminBootstrapAuthUser(authUser) && matchedUserId === ADMIN_BOOTSTRAP.username) {
                            userData = {
                            ...
                            await setDoc(doc(db, "users", matchedUserId), {
                                ...userData,
                                createdAt: Date.now(),
                                updatedAt: Date.now()
                            }, { merge: true });
                        } else {
                        if (typeof signOutFn === "function") await signOutFn(auth);
                        alert("Không tìm thấy hồ sơ người dùng. Vui lòng liên hệ admin.");
                        return;
                        }
                    } else {
                    ...
                    try {
                        matchedUser = await findUserDocByUsername(u);
                    } catch (profileErr) {
                        if (isPermissionDeniedError(profileErr)) {
                            alert(`Firestore đang chặn đăng nhập tài khoản "${normalizedUsername}". Hãy tạo document usernames/${normalizedUsername} trỏ tới email Firebase Auth, rồi publish đúng Firestore Rules.`);
                            return;
                        }
                        throw profileErr;
                    }
                    if (!matchedUser) {
                        alert(`Tài khoản này không tồn tại hoặc thiếu document usernames/${normalizedUsername}.`);
                        return;
                    }

                    matchedUserId = matchedUser.id;
                    userData = matchedUser.data || {};
                    accountEmail = normalizeEmail(userData.email);
                    ...
                    const legacyMatch = Object.prototype.hasOwnProperty.call(userData, "pass") &&
                      String(userData.pass ?? "") === String(p);
                    ...
                    try {
                        const credential = await createUserWithEmailAndPasswordFn(auth, accountEmail, p);
                        authUser = credential?.user || auth.currentUser || null;
                    } catch (createErr) {
                        ...
                        if (createCode === "auth/weak-password") {
                            ...
                            if (typeof sendPasswordResetEmailFn === "function") {
                                try {
                                    await sendPasswordResetEmailFn(auth, accountEmail);
                                    alert("Tài khoản đã có trên Firebase Auth. Hệ thống đã gửi email đặt lại mật khẩu cho bạn.");
                                } catch (resetErr) {
                                    console.warn("Could not send reset email for migrated account:", resetErr);
                                    alert("Tài khoản đã có trên Firebase Auth. Vui lòng bấm Quên mật khẩu để đặt lại.");
                                }
                            } else {
                                alert("Tài khoản đã có trên Firebase Auth. Vui lòng bấm Quên mật khẩu để đặt lại.");
                            }
                ...
                if (userData.locked) {
                    if (typeof signOutFn === "function" && authUser) await signOutFn(auth);
                    alert("Tài khoản đã bị KHÓA. Liên hệ admin!");
                    return;
                }

                if (!authUser && !requiresPasswordChange) {
                    alert("Không xác thực được phiên đăng nhập.");
                    return;
                }

                const isAdminBootstrapLogin = isAdminBootstrapAuthUser(authUser)
                    && matchedUserId === ADMIN_BOOTSTRAP.username
                    && normalizeText(userData.role) === "admin";

                if (authUser && userData.authUid && userData.authUid !== authUser.uid && !isAdminBootstrapLogin) {
                    if (typeof signOutFn === "function") await signOutFn(auth);
                    alert("Tài khoản Firebase Auth không khớp hồ sơ người dùng. Vui lòng liên hệ admin.");
                    return;
                }
                ...
                if (authUser) {
                    ...
                    legacyPasswordVerifiedForMigration = false;
                    legacyPasswordMigrationUserId = "";
                } else {
                    legacyPasswordVerifiedForMigration = true;
                    legacyPasswordMigrationUserId = matchedUserId;
                    try {
                        await updateDoc(doc(db, "users", matchedUserId), {
                            mustChangePassword: true,
                            passwordMigrationRequiredAt: Date.now()
                        });
                    } catch (migrationErr) {
                        console.warn("Could not mark password migration state:", migrationErr);
                    }
                }
                ...
                setCurrentUserSession({
                  ...
                  authUid: userData.authUid || authUser?.uid || "",
                  mustChangePassword: requiresPasswordChange
                }, matchedUserId, { persist: !requiresPasswordChange });

                alert(requiresPasswordChange
                  ? `Xin chào, ${currentUser.name}! Mật khẩu cũ quá ngắn để chuyển sang Firebase Auth. Vui lòng đổi mật khẩu mới ít nhất 6 ký tự ngay trong Hồ sơ cá nhân.`
                ...
                loadData();
                if (currentUser?.role === "admin") {
                    scheduleLegacyBackfillForAdmin();
                }
                if (requiresPasswordChange) window.showSection?.("profile");
                ...
                if (isPermissionDeniedError(e)) {
                    const normalizedUsername = normalizeText(u).toUpperCase() || "ADMIN";
                    alert(`Firestore đang báo thiếu quyền. Kiểm tra Rules đã publish chưa và tạo đủ users/${normalizedUsername}, usernames/${normalizedUsername}, authUsers/{UID}, roles/{UID}.`);
                    return;
                }
            ...
            if(!currentUser) {
                ...
                if (saved) {
                    try {
                        ...
                        const savedUser = parsed && typeof parsed === "object" ? sanitizeUserProfile(parsed) : null;
                        const authUser = auth?.currentUser || null;
                        const sameAuthUser =
                          savedUser &&
                          authUser &&
                          (
                            (savedUser.authUid && savedUser.authUid === authUser.uid) ||
                            (savedUser.email && normalizeEmail(savedUser.email) === normalizeEmail(authUser.email || ""))
                          );
                        currentUser = sameAuthUser ? savedUser : null;
                        if (currentUser) {
                          localStorage.setItem("library_user", JSON.stringify(currentUser));
                        } else {
                          localStorage.removeItem("library_user");
...
async function restoreSessionFromAuthUser(authUser) {
  if (!authUser) {
    if (legacyPasswordVerifiedForMigration) return;
    currentUser = null;
    syncState();
    localStorage.removeItem("library_user");
  ...
  try {
    let userId = "";
    const authIndexSnap = await getDoc(doc(db, "authUsers", authUser.uid));
    if (authIndexSnap.exists()) {
      userId = normalizeText(authIndexSnap.data()?.userId);
    }

    if (!userId) {
      const cached = localStorage.getItem("library_user");
      if (cached) {
        const parsed = sanitizeUserProfile(JSON.parse(cached));
        if (
          parsed.authUid === authUser.uid ||
          normalizeEmail(parsed.email) === normalizeEmail(authUser.email || "")
        ) {
          userId = normalizeText(parsed.username);
        }
      }
    }

    if (!userId) {
      const profileByEmail = await findUserDocByEmail(authUser.email || "");
    ...
    setCurrentUserSession({
      ...
      authUid: profile.authUid || authUser.uid
    }, userId);
    schedulePostLoginMaintenance({
      userId,
    ...
    console.warn("Could not restore Firebase Auth session:", err);
  }
}

if (auth && typeof onAuthStateChangedFn === "function" && !window.__libraryAuthStateSyncStarted) {
  window.__libraryAuthStateSyncStarted = true;
  onAuthStateChangedFn(auth, (authUser) => {
    restoreSessionFromAuthUser(authUser);
  });
}
...
window.handleChangePassword = async function() {
            ...
            if (newPass.length < 6) return alert("Mật khẩu Firebase Auth phải có ít nhất 6 ký tự!");
            ...
            try {
                ...
                const canCreateAuthFromLegacy =
                  !canUpdateAuthPassword &&
                  legacyPasswordVerifiedForMigration &&
                  legacyPasswordMigrationUserId === currentUser.username &&
                  currentUser.mustChangePassword &&
                  auth &&
                  typeof createUserWithEmailAndPasswordFn === "function" &&
                  isValidEmail(currentUser.email);

                if (!canUpdateAuthPassword && !canCreateAuthFromLegacy) {
                    alert("Phiên đăng nhập Firebase không hợp lệ. Vui lòng đăng xuất rồi đăng nhập lại để đổi mật khẩu.");
                    btn.innerText = oldText;
                    btn.disabled = false;
                    return;
                }
                ...
                if (canCreateAuthFromLegacy) {
                    try {
                        const credential = await createUserWithEmailAndPasswordFn(auth, normalizeEmail(currentUser.email), newPass);
                        authUser = credential?.user || auth.currentUser || null;
                    } catch (createErr) {
                        if (String(createErr?.code || "") === "auth/email-already-in-use") {
                            alert("Email này đã có trên Firebase Auth. Vui lòng đăng xuất rồi dùng Quên mật khẩu để đặt lại.");
                        } else {
                            alert("Lỗi tạo tài khoản Firebase Auth: " + createErr.message);
                        }
                        btn.innerText = oldText;
                        btn.disabled = false;
                        return;
                ...
                if (!authUser) {
                    alert("Không xác thực được phiên Firebase Auth.");
                    btn.innerText = oldText;
                    btn.disabled = false;
                    return;
                }

                // 4. C?p nh?t metadata l�n Firestore, kh�ng luu m?t kh?u
                const userRef = doc(db, "users", currentUser.username);
                await updateDoc(userRef, clearStoredPasswordPatch({
                    authUid: authUser.uid,
                    passwordUpdatedAt: Date.now(),
                    mustChangePassword: false
                }));
                await ensureUsernameIndex(currentUser.username, currentUser.email, authUser.uid, currentUser.role);
                await ensureAuthUserIndex(authUser.uid, currentUser.username, currentUser.email, currentUser.role);
                legacyPasswordVerifiedForMigration = false;
                legacyPasswordMigrationUserId = "";
                ...
                setCurrentUserSession({
                  ...currentUser,
                  authUid: currentUser.authUid || authUser.uid,
                  mustChangePassword: false
...
window.handleAddUserPanel = async function (e) {
    ...
    if (pass.length < 6) {
        if (msg) {
            msg.style.display = 'block';
            msg.textContent = "Mật khẩu Firebase Auth phải có ít nhất 6 ký tự.";
            msg.style.background = 'rgba(198,40,40,.10)';
            msg.style.color = '#b71c1c';
        }
        return;
    ...
    try {
        if (!accountCreationAuth || typeof createUserWithEmailAndPasswordFn !== "function") {
            throw new Error("Chưa cấu hình Firebase Authentication cho tạo tài khoản.");
        }
        ...
        const emailOwner = await findUserDocByEmail(normalizedEmail);
        if (emailOwner) {
        if (msg) {
            msg.style.display = 'block';
            msg.textContent = `Email "${normalizedEmail}" đã được dùng cho tài khoản khác.`;
            msg.style.background = 'rgba(198,40,40,.10)';
            msg.style.color = '#b71c1c';
        }
        return;
        ...
        const credential = await createUserWithEmailAndPasswordFn(accountCreationAuth, normalizedEmail, pass);
        const authUser = credential?.user || accountCreationAuth.currentUser || null;

        // Luu theo d�ng structure b?n dang c� tr�n Firestore, kh�ng luu m?t kh?u
        ...
        console.error(err);
        const code = String(err?.code || "");
        let text = "Lỗi tạo độc giả: " + err.message;
        if (code === "auth/email-already-in-use") text = `Email "${normalizedEmail}" đã tồn tại trên Firebase Auth.`;
        if (code === "auth/operation-not-allowed") text = "Email/Password chưa được bật trong Firebase Authentication.";
        if (code === "auth/weak-password") text = "Mật khẩu Firebase Auth phải có ít nhất 6 ký tự.";
        if (msg) {
        msg.style.display = 'block';
        ...
        if (
          ...
          try {
            ...
            console.warn("Could not clear account creation auth session:", signErr);
          }
        }
    }
    };
